...
module.exports = ( ApiaiBotkit, Botkit, app, mongoURI ) => {
  ...
  controller.hears( ['looking for a job', 'I need a job','connect me with employer'],'direct_message,direct_mention,mention', ( bot, message ) => {
    ...
    const locationsQuestion = `In what locations are you looking for a position?`
    ...
    const campusQuestion = `Were you a devMountain student? If yes type location of campus you attended, if no type "no".`
    const yearsExperienceQuestion = `How many years experience do you have?`
    ...
    askLocations = ( response, convo ) => {
      convo.ask( locationsQuestion, ( response, convo ) => {
        convo.say(`Ok the Locations I have saved is ${ response.text }`)
    ...
    askSkills = ( response, convo ) => {
      ...
      convo.say(`Ok the skills I have saved for you are ${ response.text }.`)
      askCampus( response, convo );
      convo.next();
        })
      }
    askCampus = ( response, convo ) => {
      convo.ask( campusQuestion, ( response, convo ) => {
        if( response.text ==="no"){
          askYearsExperience ( response, convo );
          convo.next();
        } else {
        convo.say(`Ok the campus I have saved for you is ${ response.text }.`)
        askYearsExperience ( response, convo )
        convo.next();
      }
      })
    ...
    askYearsExperience = ( response, convo ) => {
      convo.ask( yearsExperienceQuestion, ( response, convo ) => {
        convo.say(`Ok the years experience I have saved for you is ${ response.text }.`)
        convo.say(`Good Job! I have successfully saved your information and a recruiter will contact you if your qualifications match an open position.`)
        convo.next();
        convo.on(`end`, (convo) => {
            if(convo.status === `completed` ){

          const makeStudentObj= () => {
              ...
              const locationsArr = convo.extractResponse( locationsQuestion ).split( ", ")

              const savedEmail = convo.extractResponse(
                emailQuestion )
              const savedSkills = {};
                convo.extractReponse( skillsQuestion ).split( ", ").forEach( value => {
                  savedSkills[ value ] = value;
                } )
                console.log( "savedSkills", savedSkills );
              const savedCampus = convo.extractReponse( campusQuestion )
              const savedYearsExperience = Number( convo.extractReponse( yearsExperienceQuestion ) )
            ...
            const studentObj = {
                  "name": {
                    ...
                    "skills": savedSkills,
                    "campus": savedCampus,
                    "yearsExperience": savedYearsExperience
                  };
                  return studentObj
                }
                const createdStudentObj = makeStudentObj();

                  console.log ('studentObj', createdStudentObj )
                  new Student( studentObj ).save( ( err, newStudent ) => {
                    if ( err ) {
                      return res.status( 500 ).json( err );
                    }
                    return res.status ( 201 ).json( studentAdded )
                  } )
                }// make student obj closed
